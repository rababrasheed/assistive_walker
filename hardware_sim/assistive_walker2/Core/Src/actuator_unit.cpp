...
//connect to motor and buzzer pins
...
void ActuatorUnit::execute_alert(const ControlMessage& msg)
{
    switch (msg.command) 
    {
        ...
        case CMD_SLOW_BEEP:
            ...
            {
                buzzer_state = !buzzer_state; //flip on and off
                __HAL_TIM_SET_COMPARE(buzzer_tim, TIM_CHANNEL_1, buzzer_state ? 7 : 0); //PWM duty
                last_toggle = HAL_GetTick(); //reset timer
            }
		#else
