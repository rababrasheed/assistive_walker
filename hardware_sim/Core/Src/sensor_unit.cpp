...
bool SensorUnit::tick(uint32_t timestamp_ms)
{
    ...
    {
        msg.distance_cm= read_distance_cm();
    }
...
float SensorUnit::read_distance_cm() const
{
    ...
    // 17cm per ms (sound round trip)
    return (float)(elapsed_ms * 17);
    */
	  // bring in the shared ISR variables
	    ...
	    uint32_t elapsed_ms = echo_fall_time - echo_rise_time;
	    return (float)(elapsed_ms * 17);
}
#else
float SensorUnit::read_distance_cm() const
